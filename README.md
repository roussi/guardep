...
```bash
guardep --version    # -> guardep 0.2.0
guardep --help
```

### Verify release provenance (SLSA Build L3)

Every published artifact has a SLSA v1.0 in-toto attestation
generated by the
[`slsa-github-generator`](https://github.com/slsa-framework/slsa-github-generator)
reusable workflow. The provenance is non-forgeable: it is produced
in a separate workflow context the calling job cannot tamper with,
signed keyless via Sigstore Fulcio, and logged to Rekor.
...
ART="guardep-${TAG#v}-${TARGET}.tar.gz"

# Download the artifact and the release provenance from the same tag.
gh release download "$TAG" --repo roussi/guardep \
  --pattern "$ART" --pattern 'guardep-*.intoto.jsonl'

slsa-verifier verify-artifact \
  --provenance-path "guardep-${TAG}.intoto.jsonl" \
  --source-uri github.com/roussi/guardep \
  --source-tag "$TAG" \
  "$ART"
```

A passing run prints `PASSED: SLSA verification passed` and binds
the artifact's digest to this repo at this exact tag.
