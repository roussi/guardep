...
pub fn auto_resolve(project_root: &Path) -> Result<(Vec<PackageRef>, &'static str)> {
    let candidates: &[(&str, &dyn Fn() -> Box<dyn Resolver>)] = &[
        ...
        ("Cargo.lock", &|| Box::new(CargoLockResolver)),
    ...
    let Some(chosen_name) = chosen else {
        anyhow::bail!(
            "no supported manifest in {} (looked for package-lock.json, \
             pnpm-lock.yaml, yarn.lock, Cargo.lock, pom.xml, build.gradle, \
             build.gradle.kts)",
...
// ── Cargo.lock (v3/v4 TOML lockfile) ────────────────────────────────────

pub struct CargoLockResolver;

impl Resolver for CargoLockResolver {
    fn resolve(&self, project_root: &Path) -> Result<Vec<PackageRef>> {
...
";
    ...
    #[test]
    fn resolve_with_reads_cargo_lock_v3_registry_packages() {
        let dir = TempDir::new().unwrap();
        write(
            dir.path(),
            "Cargo.lock",
...
"#,
        ...
        let pkgs = resolve_with(dir.path(), "Cargo.lock").unwrap();

        assert_eq!(pkgs.len(), 2);
        assert_eq!(
            pkgs[0],
            PackageRef::new(Ecosystem::Cargo, "serde", "1.0.197")
        );
        assert_eq!(
            pkgs[1],
            PackageRef::new(Ecosystem::Cargo, "tokio", "1.37.0")
        );
    }

    #[test]
    fn auto_resolve_picks_cargo_lock_when_present() {
        let dir = TempDir::new().unwrap();
        write(
            dir.path(),
            "Cargo.lock",
...
"#,
        ...
        assert_eq!(kind, "Cargo.lock");
        assert_eq!(pkgs.len(), 1);
        assert_eq!(pkgs[0].name, "serde");
        assert_eq!(pkgs[0].ecosystem, Ecosystem::Cargo);
    }

    #[test]
    fn auto_resolve_treats_cargo_toml_without_lockfile_as_unsupported() {
        let dir = TempDir::new().unwrap();
        write(
            dir.path(),
            "Cargo.toml",
    ...
    #[test]
    fn resolve_with_errors_when_cargo_lock_is_missing() {
        let dir = TempDir::new().unwrap();

        let err = resolve_with(dir.path(), "Cargo.lock").unwrap_err();
        let msg = format!("{err}");

        assert!(msg.contains("lockfile Cargo.lock not found"));
    }
